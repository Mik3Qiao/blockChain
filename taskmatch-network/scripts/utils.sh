...
chaincodeInvoke() {
  ...
  peer chaincode invoke -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n taskmatching -c '{"Args":["createTaskMatching", "work", "[[1,2,3],[4,5,6],[7,8,9]]"]}'
  res=$?
  set +x
